...
idempotentes de la base SQLite de CNBPEO.
...
def recalcular_todas_las_factibilidades() -> int:
    ...
    try:
        ...
        # El módulo de factibilidad/indicadores se retiró: no hay nada que recalcular.
        return 0
...
# ---------------------------------------------------------------------------
# Autodiagnóstico de Calidad (Código Nacional de Buenas Prácticas)
# ---------------------------------------------------------------------------
# Herramienta de autoevaluación institucional, independiente del dominio de
# Indicadores/Fuentes/Factibilidad de arriba: no participa del Engine de
# factibilidad ni de las vistas resueltas de indicadores. Ver
# data/autodiagnostico_catalogo.py para el origen y estructura de los datos
# (Matriz de Autodiagnóstico para la Calidad de la Producción Estadística,
# ONE, versión 004) y models/crud_autodiagnostico.py para las operaciones de
# lectura/escritura que usa views/autodiagnostico.py.
#
# Jerarquía: Nivel (3: GEI/GPE/GRE) > Principio (15) > Elemento (67).
#
# NOTA SOBRE GRANULARIDAD: el Excel original desglosa cada uno de estos 67
# "Requisitos de cumplimiento" en, en promedio, ~4 "Código de elemento" (252
# en total, p. ej. GEI-1.1.1, GEI-1.1.2 bajo el requisito GEI-1.1) y marca
# SI/NO/N/A + Nivel de Cumplimiento en ESE nivel más fino, no en el de
# Requisito. Por decisión explícita del cliente (más simple de llenar para
# este prototipo), aquí se marca cumplimiento una vez por cada uno de los 67
# Elementos (= Requisito de cumplimiento del Excel), no por cada uno de los
# 252 códigos de verificación — el texto de esos códigos se conserva como
# referencia de apoyo en autodx_elementos.detalle, pero no se puntúa por
# separado. El % de cumplimiento calculado aquí (ver
# models.crud_autodiagnostico.calcular_resumen) por lo tanto NO es
# numéricamente idéntico al de la hoja RESULTADOS del Excel (que promedia
# sobre 252 filas, no sobre 67), aunque replica la misma fórmula: (Iniciado +
# Parcial + Total) / (No Iniciado + Iniciado + Parcial + Total) × 100,
# excluyendo "No aplica" del denominador, promediado por principio y luego
# por nivel — ver hoja Formulas/RESULTADOS del Excel de referencia.


def migrar_autodiagnostico_tablas() -> None:
    """Crea las tablas del Autodiagnóstico de Calidad si no existen."""
    with conexion_transaccional() as (conn, cursor):
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS autodx_niveles (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                codigo TEXT UNIQUE NOT NULL,
                letra TEXT NOT NULL,
        ...
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS autodx_principios (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                nivel_id INTEGER NOT NULL,
                numero INTEGER UNIQUE NOT NULL,
                nombre TEXT NOT NULL,
                orden INTEGER NOT NULL,
                FOREIGN KEY (nivel_id) REFERENCES autodx_niveles(id) ON DELETE CASCADE
            )
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS autodx_elementos (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                principio_id INTEGER NOT NULL,
                codigo TEXT UNIQUE NOT NULL,
                numero TEXT NOT NULL,
                requisito_cumplimiento TEXT NOT NULL,
                detalle TEXT,
                orden INTEGER NOT NULL,
                FOREIGN KEY (principio_id) REFERENCES autodx_principios(id) ON DELETE CASCADE
            )
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS autodx_evaluaciones (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                elemento_id INTEGER UNIQUE NOT NULL,
        ...
        """)
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_autodx_principios_nivel_id
            ON autodx_principios(nivel_id)
        """)
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_autodx_elementos_principio_id
            ON autodx_elementos(principio_id)
        """)
    logger.info("Tablas de Autodiagnóstico de Calidad verificadas/creadas correctamente.")


def sembrar_autodiagnostico() -> None:
    """Siembra el catálogo fijo (3 niveles, 15 principios, 67 elementos) del
    Código Nacional de Buenas Prácticas — ver data/autodiagnostico_catalogo.py.

    Idempotente vía upsert (ON CONFLICT) sobre las claves UNIQUE
    (codigo/numero): correr esto de nuevo tras una actualización del
    catálogo actualiza los textos sin duplicar filas ni pisar evaluaciones
    ya cargadas por el usuario (autodx_evaluaciones no se toca aquí salvo
    para agregar la fila 'No Iniciado' de arranque a un elemento nuevo).
    Además, siembra una fila en autodx_evaluaciones para cada elemento que
    todavía no tenga una (default 'No Iniciado') para que las estadísticas
    de % de cumplimiento tengan siempre el denominador completo de 67 desde
    el primer arranque, no solo los que el usuario ya haya tocado.
    """
    from data.autodiagnostico_catalogo import ELEMENTOS, NIVELES, PRINCIPIOS

    with conexion_transaccional() as (conn, cursor):
        for codigo, letra, nombre, descripcion, orden in NIVELES:
            cursor.execute(
                """
                INSERT INTO autodx_niveles (codigo, letra, nombre, descripcion, orden)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(codigo) DO UPDATE SET
                    letra = excluded.letra, nombre = excluded.nombre,
        ...
        nivel_ids = {
            r[0]: r[1]
            for r in cursor.execute("SELECT codigo, id FROM autodx_niveles").fetchall()
        }

        for numero, nivel_codigo, nombre, orden in PRINCIPIOS:
            cursor.execute(
                """
                INSERT INTO autodx_principios (nivel_id, numero, nombre, orden)
                VALUES (?, ?, ?, ?)
                ON CONFLICT(numero) DO UPDATE SET
                    nivel_id = excluded.nivel_id, nombre = excluded.nombre, orden = excluded.orden
                """,
                (nivel_ids[nivel_codigo], numero, nombre, orden),
            )

        principio_ids = {
            r[0]: r[1]
            for r in cursor.execute("SELECT numero, id FROM autodx_principios").fetchall()
        }

        for codigo, _nivel_codigo, principio_numero, numero, requisito, detalle, orden in ELEMENTOS:
            cursor.execute(
                """
                INSERT INTO autodx_elementos
                    (principio_id, codigo, numero, requisito_cumplimiento, detalle, orden)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(codigo) DO UPDATE SET
                    principio_id = excluded.principio_id, numero = excluded.numero,
                    requisito_cumplimiento = excluded.requisito_cumplimiento,
                    detalle = excluded.detalle, orden = excluded.orden
                """,
                (principio_ids[principio_numero], codigo, numero, requisito, detalle, orden),
            )

        cursor.execute("""
            INSERT INTO autodx_evaluaciones (elemento_id, nivel_cumplimiento)
            SELECT e.id, 'No Iniciado'
            FROM autodx_elementos e
            WHERE NOT EXISTS (
                SELECT 1 FROM autodx_evaluaciones ev WHERE ev.elemento_id = e.id
            )
        """)

    logger.info(
        "Catálogo de Autodiagnóstico sembrado: %d niveles, %d principios, %d elementos.",
        len(NIVELES), len(PRINCIPIOS), len(ELEMENTOS),
    )
...
def inicializar_base_datos() -> None:
    ...
    if not _migracion_ya_aplicada(_clave_recalculo):
        try:
            ...
            logger.info("Recálculo de factibilidad omitido (módulo retirado).")
...
MIGRACIONES: list[Callable[[], None]] = [
    ...
    migrar_autodiagnostico_tablas,
    sembrar_autodiagnostico,
]
