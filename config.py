...
config.py
=========
Fuente única de verdad para la configuración global de CNBPEO.
...
CNBPEO_ENV: str = os.environ.get("CNBPEO_ENV", "dev")

# Ruta a la base de datos SQLite. Configurable vía CNBPEO_DB_PATH para
...
_ruta_db_nueva = os.path.join(BASE_DIR, "CNBPEO.db")
_ruta_db_legado = os.path.join(BASE_DIR, "sidoe.db")
# Migración transparente del nombre de la base de datos: si todavía existe la
# BD anterior (sidoe.db) y aún no la nueva (CNBPEO.db), se renombra para
# CONSERVAR todos tus datos sin perder nada.
if os.path.exists(_ruta_db_legado) and not os.path.exists(_ruta_db_nueva):
    try:
        os.rename(_ruta_db_legado, _ruta_db_nueva)
    except OSError:
        _ruta_db_nueva = _ruta_db_legado  # si no se pudo renombrar, se sigue usando la anterior
DB_PATH = os.environ.get("CNBPEO_DB_PATH", _ruta_db_nueva)
...
# Configurable vía CNBPEO_UMBRAL_ELIMINACIONES_AUTOBLOQUEO — default idéntico
# al valor fijo anterior si la variable de entorno no está definida.
UMBRAL_ELIMINACIONES_AUTOBLOQUEO: int = int(
    os.environ.get("CNBPEO_UMBRAL_ELIMINACIONES_AUTOBLOQUEO", "5")
)
